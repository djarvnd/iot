...
from pymysql import *
conn = pymysql.connect(host="localhost",user="root",password="13252EC008",db="login_data")
cursor = conn.cursor()
...
class RegisterForm(FlaskForm):
	...
	college=StringField('college:',validators=[InputRequired(),Length(min=5,max=15,message="characters should be between 5 and 15")])
	email=StringField('email:',validators=[InputRequired(),Email(message="enter a valid email address")])
	username=StringField('username:',validators=[InputRequired(),Length(min=5,max=15,message="characters should be between 5 and 15")])
...
def check():
	...
	if form.validate_on_submit():
		...
		cursor.execute("SELECT email FROM aswini")
		d=cursor.fetchall()
		...
		for row in d:
			if row[0]==k:
				i=i+1
		if i==0:
			cursor.execute("INSERT INTO aswini values('%s','%s','%s','%s','%s','%s')" %(fname,lname,college,email,username,password))
			form=LoginForm()
...
def log1():
	...
	if i==0:
		...
		cursor.execute("SELECT pword FROM aswini where email='" + email1 + "'")
		p=cursor.fetchall()
		for row in p:
			if row[0]==password:
				session['user']=email1
...
def getsession():
	form=LoginForm()
	if 'user' in session:
		cursor.execute("SELECT * FROM aswini where email='" + session['user'] + "'")
		p=cursor.fetchone()
		print(p)
		print(cursor.execute("DESCRIBE aswini"))
